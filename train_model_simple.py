...
def build_model(input_data_tensor, input_label_tensor, train_mode, drop_out, batch_norm):
    ...
    loss_weight_decay = tf.reduce_sum(tf.stack([tf.nn.l2_loss(i) \
...
def train(trn_data, tst_data=None):
    ...
    steps_per_epoch = num_samples_per_epoch // (worker_number * microbatch_size)
    max_lr = learning_rate * (worker_number * microbatch_size) / 200
    ...
    with G.as_default():
        ...
        model = build_model(input_data_tensor, input_label_tensor, \
                    train_mode, drop_out, batch_norm)
        ...
        for epoch in range(1, num_epochs+1):
            ...
            for step in range(steps_per_epoch):
                ...
                for worker in range(worker_number):
                    ...
                    ops = [var_list, clipped_grads] + [model[k] for k in sorted(model.keys())]
                    ...
                    ##########################################################
                    # Print out var and grad norm and the ratio of the norm
                    ##########################################################
                    # var_and_grad = zip(results[0], results[1])
                    # name_and_vg = zip(var_name_list, var_and_grad)
                    # norm = [[name, np.linalg.norm(np.asarray(value[0])), np.linalg.norm(np.asarray(value[1]))] \
                    #         for name, value in name_and_vg]
                    #
                    # for elem in norm:
                    #     print '%s, var_norm = %f, grad_norm = %f, ratio = %f' \
                    #             % (elem[0], elem[1], elem[2], elem[1]/elem[2])

                    grad_workers.append([grad for grad in results[1]])
                ...
                # name_and_gradworkers = zip(var_name_list, grad_workers)
                # for grad in name_and_gradworkers:
                #     print grad[0]
                #     variance = np.var(np.asarray(grad[1]), axis=0)
                #     avg_variance = np.mean(variance)
                #     maximum = np.max(variance)
                #     minimum = np.min(variance)
                #     info = [avg_variance, maximum, minimum]
                #     print info
                # print name_and_gradworkers[1]
